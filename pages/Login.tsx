...
import { Diamond, ArrowRight, Globe, Eye, EyeOff, Briefcase, User, ArrowLeft } from 'lucide-react';
      ...
      <div className="relative z-10 w-full max-w-md px-6 py-12">
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-4xl text-cream mb-2">
            {isForgotPasswordMode ? 'Account Recovery' : isLoginMode ? 'Welcome Back' : 'Join Stylus'}
          </h1>
          <p className="text-golden-orange text-xs uppercase tracking-[0.2em]">
            {isForgotPasswordMode ? 'Secure Password Reset' : isLoginMode ? 'Enter the Inner Circle' : 'Create Your Account'}
          </p>
        </div>
        ...
        <div className="bg-[#1f0c05]/80 backdrop-blur-md border border-white/10 p-8 shadow-2xl rounded-sm">
          ...
          {isForgotPasswordMode ? (
              <form onSubmit={handlePasswordReset} className="space-y-6">
                  <div className="bg-golden-orange/10 border border-golden-orange/30 p-4 rounded text-center">
                      <p className="text-cream text-sm">Enter the email associated with your account and we'll send you a link to reset your password.</p>
                  </div>
                  ...
                  <Button fullWidth disabled={isLoading} className="mt-4">
                        {isLoading ? <span className="animate-pulse">Sending Request...</span> : "Send Reset Link"}
                  </Button>

                  <button 
                    type="button" 
                    onClick={() => setIsForgotPasswordMode(false)}
                    className="w-full text-xs text-cream/50 hover:text-white flex items-center justify-center gap-2 mt-2 pt-2"
                  >
                        ...
                        <div className="flex justify-between mb-2">
                            ...
                            {isLoginMode && (
                                <button 
                                    ...
                                    className="text-[10px] text-golden-orange hover:text-white uppercase tracking-wider transition-colors font-bold"
                                >
                                    Forgot Password?
                                </button>
                            )}
